...
function babu_manager_update_target() {
    ...
    if (!babu_manager_check_table()) {
        error_log('Babu Update Target Error: Table check failed');
        return;
    }
    ...
    $current_target = $wpdb->get_var("SELECT setting_value FROM $settings_table WHERE setting_key = 'babu_target'");
    $max_target = $current_target && strpos($current_target, '/') !== false ? explode('/', $current_target)[1] : 100;
    ...
    if ($exists) {
        ...
        if ($result === false) {
            error_log('Babu Update Target Error: Failed to update target to ' . $new_target . ', Error: ' . $wpdb->last_error);
        }
    } else {
        $result = $wpdb->insert($settings_table, ['setting_key' => 'babu_target', 'setting_value' => $new_target]);
        if ($result === false) {
            error_log('Babu Update Target Error: Failed to insert target ' . $new_target . ', Error: ' . $wpdb->last_error);
        }
    }
}
